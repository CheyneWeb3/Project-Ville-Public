/**
 *Submitted for verification at testnet.bscscan.com on 2025-12-22
*/
...
// File: https://github.com/ArielRin/InHaus-Multichain-Staking-System-Public-Files/blob/main/Contracts/IERC6551.sol
...
// File: Cashier2025/NFT6551/ERC6551Bytecode.sol
...
library ERC6551Bytecode {
    ...
    function codeAt(
        ...
        assembly {
            ...
            mstore(code, size)
            extcodecopy(account, add(code, 0x20), start, size)
        }
    }
}

// File: Cashier2025/NFT6551/TbaAccountsV3final.sol
...
contract BudVilleERC6551Account is IERC165, IERC1271, IERC6551Account {
    using SafeERC20 for IERC20;
    using MessageHashUtils for bytes32;

    event EthSent(address indexed to, uint256 amount);
    event OwnershipTransferredViaTBA(address indexed contractAddress, address indexed newOwner);
    ...
    function executeBatch(
        ...
        require(msg.sender == owner(), "Not token owner");
        require(targets.length == values.length && values.length == payloads.length, "Length mismatch");
    ...
    function extcall(address to, uint256 value, bytes calldata data) external payable {
        require(msg.sender == owner(), "Not token owner");
    ...
    function executeNested(
        ...
        for (uint256 i = 0; i < proof.length; i++) {
            address next = computeAddress(proof[i]);
            require(current == next, "Proof invalid at step");
            current = next;
        }
    ...
    function computeAddress(NestedProof memory proof) internal pure returns (address) {
        bytes32 codeHash = keccak256(
            ERC6551Bytecode.generateBytecode(
    ...
    function eoaCall(address to, uint256 value, bytes calldata data) external payable returns (bytes memory result) {
        (bool success, bytes memory returnData) = to.call{value: value}(data);
    ...
    function eoaExecute(
        ...
        bytes32 hash = keccak256(abi.encodePacked(to, value, data, address(this))).toEthSignedMessageHash();
        require(SignatureChecker.isValidSignatureNow(nftOwner, hash, signature), "Invalid signature");
    ...
    function withdrawAll(
        ...
        require(msg.sender == owner(), "Not token owner");
        require(erc20Tokens.length == erc20Amounts.length, "Length mismatch");

        if (nativeAmount > 0) {
            require(address(this).balance >= nativeAmount, "Insufficient ETH balance");
            (bool success, ) = receiver.call{value: nativeAmount}("");
            require(success, "ETH withdrawal failed");
            emit EthSent(receiver, nativeAmount);
        }

        for (uint256 i = 0; i < erc20Tokens.length; i++) {
            IERC20 token = IERC20(erc20Tokens[i]);
            uint256 balance = token.balanceOf(address(this));
            require(balance >= erc20Amounts[i], "Insufficient token balance");
            token.safeTransfer(receiver, erc20Amounts[i]);
        }
    }


    function withdrawAllERC721(address[] calldata nftContracts, uint256[] calldata tokenIds) external {
        require(msg.sender == owner(), "Not token owner");
        require(nftContracts.length == tokenIds.length, "Length mismatch");
        for (uint256 i = 0; i < nftContracts.length; i++) {
            IERC721(nftContracts[i]).safeTransferFrom(address(this), owner(), tokenIds[i]);
        }
    }

    function withdrawAllERC1155(
        ...
        require(
            ...
            amounts.length == datas.length,
            "Length mismatch"
        );
        for (uint256 i = 0; i < nftContracts.length; i++) {
            IERC1155(nftContracts[i]).safeTransferFrom(address(this), owner(), tokenIds[i], amounts[i], datas[i]);
        }
    }
    ...
    function transferERC1155(address tokenContract, address to, uint256 tokenId, uint256 amount, bytes calldata data) external {
        require(msg.sender == owner(), "Not token owner");
        IERC1155(tokenContract).safeTransferFrom(address(this), to, tokenId, amount, data);
    }


    function contractTransferEoa(address contractToSend, address newOwner) external {
        require(msg.sender == owner(), "Not token owner");
        require(newOwner != address(0), "Invalid new owner");
        (bool success, ) = contractToSend.call(
            abi.encodeWithSignature("transferOwnership(address)", newOwner)
        );
        require(success, "Ownership transfer failed");
        emit OwnershipTransferredViaTBA(contractToSend, newOwner);
    }

    function nftInfo() external view returns (uint256 chainId, address tokenContract, uint256 tokenId) {
        uint256 length = address(this).code.length;
        return abi.decode(ERC6551Bytecode.codeAt(address(this), length - 0x60, length), (uint256, address, uint256));

    }

    function token() external view returns (address, uint256) {
        (, address tokenContract, uint256 tokenId) = this.nftInfo();
